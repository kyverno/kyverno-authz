...
func TestLib_GetTypedArgumentOverloadsRejectNonStringKeys(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "GetIntSliceArgument",
			expr: `mcp.Parse("{}").GetIntSliceArgument(0, 0)`,
		},
		{
			name: "GetFloatSliceArgument",
			expr: `mcp.Parse("{}").GetFloatSliceArgument(0.0, 0.0)`,
		},
		{
			name: "GetBoolSliceArgument",
			expr: `mcp.Parse("{}").GetBoolSliceArgument(false, false)`,
		},
	}
